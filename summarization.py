...
from transformers import AutoModelForSeq2SeqLM, AutoModelForSeq2SeqLM
...
class Summarizer(pl.LightningModule):
    ...
    def __init__(self, params):
        ...
        self.args = params
        self.hparams = params
        self.tokenizer = LongformerTokenizer.from_pretrained("./longformer-bartpho-word-base-16384", use_fast=True)

        if 'long' in self.args.model_path:
            print('Process in if')
            config = LongformerEncoderDecoderConfig.from_pretrained('./longformer-bartpho-word-base-16384', output_hidden_states=True)
            config.attention_dropout = self.args.attention_dropout
            print("config:", config.attention_dropout)
            ...
            print("diliation: ", config.attention_dilation)
            self.model = LongformerEncoderDecoderForConditionalGeneration.from_pretrained('./longformer-bartpho-word-base-16384', config=config)
            print('done if')
        else:
    ...
    def add_model_specific_args(parser, root_dir):
        ...
        parser.add_argument("--model_path", type=str, default='./longformer-bartpho-word-base-16384',
                            help="Path to the checkpoint directory or model name")
        parser.add_argument("--tokenizer", type=str, default='./longformer-bartpho-word-base-16384')
        ...
        parser.add_argument("--attention_window", type=int, default=128, help="Attention window")
...
def main(args):
    ...
    model.hf_datasets = load_dataset("HaiLong9901/VNeseTextSum")
    ...
    args.dataset_size = 2000 # hardcode dataset size. Needed to compute number of steps for the lr scheduler
